...
async def get_transfer(request, transfer_id: int):
...
async def create_transfer(request, payload: TransferCreateSchema):
    ...
    result, error = await create_transfer_record()
    if error:
        return error_response(error, code=400)
    return success_response(
        format_transaction(result), "Transfer completed successfully"
    )


@router.delete("/{transfer_id}", response=TransactionResponse)
async def delete_transfer(request, transfer_id: int):
    ...
    success, error = await soft_delete_transfer()
    if not success:
