...
# --- Streamlit Chat UI -----------------------------------------------------
st.set_page_config(page_title="Legal Advisor Agent", layout="centered")
st.title("⚖️ Egypt Fintech Legal Advisor")

kb = load_kb(KB_JSON_PATH)
try:
    index, ids = load_index()
except:
    index, ids = build_index(kb) if kb else (None, [])
...
if prompt := st.chat_input("Ask your legal/compliance question..."):
    ...
    retrieved = retrieve_with_rewriting(prompt, kb, index, ids) if index else []
    raw = call_llm(prompt, retrieved, lang, st.session_state.history)
    ...
    if parsed:
        ...
        if parsed.get("sources"):
            srcs = "\n".join(
                [f"- [{s['title']}]({s['source_url']}) (relevance {s.get('confidence',0):.2f})" for s in parsed["sources"] if s.get("source_url")]
            )
            st.chat_message("assistant").markdown("**Sources:**\n" + srcs)
        save_to_csv(prompt, parsed)
    else:
        st.chat_message("assistant").markdown(raw)
        st.session_state.history.append({"role": "assistant", "content": raw})
        save_to_csv(prompt, {"answer_text": raw, "language": lang})
